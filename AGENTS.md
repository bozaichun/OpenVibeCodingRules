...
1. 根据依赖与入口文件判断语言（见 §1）
2. 前端仓再检查是否存在 `public/plugin.json`（§0.1）
3. 加载 `rules/CodeConduct.md` + `rules/CodingSpec/QualityBaseline.md` + 对应语言目录下的 `CodingSpec.md`
4. 仅前端 UI 任务再加载 `DESIGN.md` 与 `WebVariable/`
...
| `rules/CodingSpec/Python/CodingSpec.md` | Python · FastAPI / Django | Python 业务代码 |
| `rules/CodingSpec/Java/CodingSpec.md` | Java · Spring Boot | Java 业务代码 |
| `rules/CodingSpec/Go/CodingSpec.md` | Go · Gin | Go 业务代码 |
| `rules/CodingSpec/Node.js/CodingSpec.md` | Node.js · NestJS / Express | Node **服务端**业务代码 |
| `rules/CodingSpec/PHP/CodingSpec.md` | PHP · Laravel | PHP 业务代码 |
| `rules/CodingSpec/C/CodingSpec.md` | C · 系统 / 嵌入式 | C 业务 / 基础库代码 |
| `rules/CodingSpec/C++/CodingSpec.md` | C++ · 现代 C++ | C++ 业务 / 原生模块 |
| `rules/CodingSpec/CSharp/CodingSpec.md` | C# · ASP.NET Core | .NET 业务代码 |
| `rules/CodingSpec/Rust/CodingSpec.md` | Rust · Axum / Actix | Rust 业务代码 |
| `rules/CodingSpec/SQL/CodingSpec.md` | SQL · 迁移 / 建模 | 库表与查询 |
| `rules/CodingSpec/R/CodingSpec.md` | R · tidyverse / Shiny | 数据分析 / Shiny |
| `rules/CodingSpec/Kotlin/CodingSpec.md` | Kotlin · Ktor / Android | Kotlin 业务代码 |
| `rules/CodingSpec/Swift/CodingSpec.md` | Swift · SwiftUI / Vapor | Swift 业务代码 |
| `rules/CodingSpec/Dart/CodingSpec.md` | Dart · Flutter | Flutter 业务代码 |
| `rules/CodingSpec/Ruby/CodingSpec.md` | Ruby · Rails | Ruby 业务代码 |
| `rules/CodingSpec/Scala/CodingSpec.md` | Scala · Play / ZIO | Scala 业务代码 |
| `rules/CodingSpec/Shell/CodingSpec.md` | Shell · Bash | 脚本 / CI / 运维自动化 |

**选用原则**

1. 行为与范围 → `CodeConduct.md`
2. 质量 / 门禁 / 接口 / 技术债 → `QualityBaseline.md`
3. 怎么写（语言落点） → `rules/CodingSpec/<语言>/CodingSpec.md`
4. 前端长什么样 → `DESIGN.md` + **WebVariable** CSS
...
| 任务类型 | 应加载 | 通常不必加载 |
|---------|--------|-------------|
| 任意编码起步 | `AGENTS.md` · `CodeConduct.md` | 其他语言 CodingSpec |
| 任意业务代码 | + `QualityBaseline.md` + 对应语言 `CodingSpec.md` | 无关语言规范 |
| 前端业务代码 | + `JavaScript&TypeScript/CodingSpec.md` | DESIGN（无 UI 时） |
| 前端 UI / 样式 | + `DESIGN.md` · 相关 WebVariable | PreView HTML 全文 |
...
| 间距 / 字号 / 阴影 | `--size-16` · `--fs-14` · `--shadow` | SystemVariable |
...
编写业务代码时**必须**遵守 `rules/CodingSpec/QualityBaseline.md`，摘要如下：
...
- [ ] 已识别语言与框架，并加载 `QualityBaseline` + 对应 `CodingSpec`
...
*最后同步：rules/CodeConduct.md · QualityBaseline · rules/CodingSpec/** · WebVariable · PreView · puffseed*
