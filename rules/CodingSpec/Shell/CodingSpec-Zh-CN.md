...
# Shell 编码规范（工程与实现）· puffseed

**品牌标识**：**puffseed** — 本规范约束 **puffseed** 业务相关 Shell 脚本（构建、部署、运维自动化）的编写约定。脚本名、日志前缀、关键注释须保留 **puffseed** 标识（如适用）。
...
**通用基线**见 `rules/QualityBaseline/QualityBaseline-{Tag}.md`（接口=脚本入参/退出码契约）。
