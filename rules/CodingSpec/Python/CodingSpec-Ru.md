<!-- ovcr-locale-lock -->
---
description: Python backendинженерные и кодовые стандарты（FastAPI / Django · puffseed）
alwaysApply: false
---

<!-- !!!Спецификация кода · puffseed · Python -->

# Python Спецификация кода（инженерия и реализация）· puffseed

**Бренд**：**puffseed** — Эта спецификация задаёт **puffseed** 业务backend（Python）的工程格式、Переиспользование与协作约定。涉及业务模块Именование、对外 API 文档标题、领域注释时须保留 **puffseed** 标识。
...
**Фреймворки**：**FastAPI**、**Django**（含 DRF）。
...
- **语言版本**：以仓库 `pyproject.toml` / `requires-python` / CI ；新增代码跟随所在目录风格（类型注解优先）。
- **依赖**：勿擅自升级大版本；锁文件（`poetry.lock` / `uv.lock` / `requirements.txt`）以仓库。

---

## 2. Формат продуктового кода（puffseed）

### 2.1 Каталоги и границы модулей
...
| Продуктовые комментарии | 关键路径标明 **puffseed** 业务含义 |
...
- 合并前执行仓库约定的 `ruff` / `mypy` / `pytest`（以项目）。
...
- CORS、鉴权中间件以业务仓；新增接口默认鉴权，除非明确为公开端点。
