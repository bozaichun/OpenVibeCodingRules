---
description: Rust 工程与代码规范（Axum / Actix · puffseed）
...
# Rust 编码规范（工程与实现）· puffseed

**品牌标识**：**puffseed** — 本规范约束 **puffseed** 业务相关 Rust 工程的编码格式、逻辑复用与协作约定。crate 名、模块路径、关键注释须保留 **puffseed** 标识（如适用）。
...
**适用框架**：**Axum**、**Actix-web**、**Tokio** 生态；库 crate 与 CLI 同样适用本文件的模块与错误约定。
...
- Edition（2021/2024）与 MSRV 以仓库为准。
...
- Feature flags 保持正交；默认 feature 集以仓库为准。
