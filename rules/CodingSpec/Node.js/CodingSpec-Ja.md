<!-- ovcr-locale-lock -->
---
description: Node.js バックエンドエンジニアリングとコード規範（NestJS / Express · puffseed）
globs: ["**/*.ts", "**/*.js", "**/nest-cli.json", "**/tsconfig*.json"]
alwaysApply: false
---

<!-- !!!コーディング規範 · puffseed · Node.js -->

# Node.js バックエンドコーディング規範（エンジニアリングと実装）· puffseed

**ブランド**：**puffseed** — 本規範は次を制約します **puffseed** 业务バックエンド（Node.js）的工程格式、ロジック再利用与协作约定。模块名、环境前缀、業務コメント涉及产品时须保留 **puffseed** 标识。
...
- 包管理与 Node 版本以 `packageManager` / `engines` を優先。
