...
扩展 Token 时：**主题色/文本色** 写入 `ThemeVariable.css`；**尺寸/间距** 写入 `SystemVariable.css`；**重置规则** 仅在确有全局需求时改 `ProjectReset.css`。
...
| 类别 | 变量示例 | 用途 |
|------|---------|------|
| 间距 | `--size-8` · `--size-16` · `--size-24` · `--size-40` · `--size-56` | 元素/模块/页面边距、页头高度等 |
| 字号 / 行高 | `--fs-12`/`--lh-20` · `--fs-14`/`--lh-22` · `--fs-16`/`--lh-24` … | 字阶与行高成对引用，见 SubjectAuthority §1.3 |
| 图标 | `--wh-16` · `--wh-24` · `--wh-32` | 字体图标 / 卡片小图标 / 常用按钮大图标 |
...
2. **字阶 + 行高成对**：使用 `--fs-*` 时必须同步 `--lh-*`（见 `SubjectAuthority.md` §1.3），相符相承、缺一不可
3. **样式 1:1**：组件 / 页面 / 布局样式各在其作用域内
4. **过渡**：`transition: ... 0.15s~0.2s ease`
5. **组件样式隔离**：Vue scoped / React CSS Modules / Angular 组件样式，避免污染全局
6. **框架约定**：业务处理函数 `handle` 前缀；副作用在卸载时清理

### 6.2 禁止做

1. 硬编码主题色值（代码块固定深色底除外）
2. 只设字阶不设对应行高，或字阶与行高错配
3. 在业务组件或项目中重复定义已在 WebVariable 中的 Token
4. 绕过已定稿的全局 CSS 架构私自引入第二套色板
5. 擅自修改 `rules/CodingSpec/JavaScript&TypeScript/WebVariable/` 核心文件（扩展套件除外且须开发者明确要求）
