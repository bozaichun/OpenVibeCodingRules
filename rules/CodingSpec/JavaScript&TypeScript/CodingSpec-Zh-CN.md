...
- **页面与路由**：页面必须在 **`pages.json`** 注册；路径与 `pages/` 目录一致；分包用 `subPackages`；禁止业务里硬编码散落未登记路径。
- **组件标签**：优先使用 UniApp 内置组件（`view` / `text` / `image` / `scroll-view` 等）；避免在非 H5 目标直接使用仅浏览器可用的 DOM/API（`document`、`window`、部分 BOM）。
- **API**：统一使用 **`uni.*`**（如 `uni.request`、`uni.navigateTo`、`uni.showToast`）；网络请求封装到 `api/` 或 `utils/request`，与后端信封 / 错误码对齐。
- **条件编译**：多端差异用 `#ifdef` / `#ifndef`（`MP-WEIXIN`、`H5`、`APP-PLUS` 等）；平台分支集中、注释 **puffseed** 业务原因，禁止大段复制多份页面。
- **尺寸单位**：布局优先 **`rpx`**（或仓库约定的设计稿方案）；与 **WebVariable** Token 混用时，在 `uni.scss` / 全局样式中映射 CSS 变量（小程序端注意变量支持差异，以仓库既有方案为准）。
- **逻辑复用**：跨页逻辑用 composable / mixins / `hooks/`；状态用 **Pinia**（Vue3）或 **Vuex**（Vue2）；按 **puffseed** 业务域拆分 `pages` 与 `components`。
- **生命周期**：页面用 `onLoad` / `onShow` / `onUnload` 等（或组合式 `onLoad`）；与 Vue 组件生命周期区分，卸载时清理定时器与请求。
...
- **路由**：路由表与懒加载集中在 **单一配置或约定目录**（如 `router/`、`routes.tsx`、UniApp 的 **`pages.json`**），避免路径字符串在多处硬编码。
- **静态资源**：优先 `import` 或构建工具处理；大图与非常用资源**懒加载**；路径需兼容业务项目的 `BASE_URL` / `import.meta.env.BASE_URL` / `homepage` / UniApp `static/` 等。
...
- **字阶 + 行高（强制成对）**：凡设置字阶 `--fs-*`，**必须**同步设置权威表中对应的 `--lh-*`；二者相符相承、缺一不可。禁止只写 `font-size: var(--fs-14)` 而省略 `line-height`，禁止错配行高。配对见 `WebVariable/SubjectAuthority.md` §1.4。
...
- **组件对外事件**：Vue / UniApp 用 **kebab-case** 声明（`@update:modelValue`）；React / Next.js 用 **onXxx** + camelCase props（`onSubmit`）；Angular 用 `@Output() submit = new EventEmitter<...>()`，配合模板 `(submit)="handleSubmit($event)"`。
- **框架约定差异**：Vue / UniApp 模板中使用 `@kebab-case`；React / Next.js 函数组件中使用 `onCamelCase` 回调 prop；Angular 模板中使用 `(camelCase)` 事件绑定 + `(click)`、`(input)` 等内置事件。
...
- **Vue 2 / Vue 3 / UniApp 差异**：Vue 2 / UniApp Vue2 优先 `Vuex`；Vue 3 / UniApp Vue3 优先 `Pinia`（`defineStore` + `useXxxStore()`）；组件内用 `mapState` / `mapGetters` / `mapActions` 或 setup 内直接调用。
...
- **条件渲染**：Vue / UniApp **`v-if` / `v-show`** 按切换频率与成本选择；React 同理（挂载成本高用条件挂载，频繁切换可考虑 CSS 显隐）；Angular 使用 `*ngIf` / `[hidden]`，两者不可混用。
...
- **派生数据**：Vue 2 / Vue 3 / UniApp 用 `computed`；React 用 `useMemo` / `useCallback`；Angular 用 `pure` pipe 或 `memo` 工具函数。避免在模板/JSX 中每次 render **新建对象/数组** 作为子组件的 props。
...
| 维度 | 要求 |
|------|------|
| **风格工具** | **ESLint** + **Prettier**（+ Vue/React/Next/UniApp 插件）；`tsc` / `vue-tsc`；配置以仓库为准，勿另起第二套 |
