...
| 文件 | 职责 | 谁改 |
|------|------|------|
| **`SubjectAuthority.md`** | **权威源**：主题色、文本色、间距、**布局尺寸**、**字阶+行高**、图标、边框、阴影的表格定义 | 设计师 / 开发者先改这里 |
| **`ThemeVariable.css`** | 主题色 · 功能色 · 辅助色 · 文本色（由权威表同步） | 由 Agent / 开发者按表生成，勿手改漂移 |
| **`SystemVariable.css`** | 间距 · **布局 `--layout-*`** · 图标 · **字阶 `--fs-*` + 行高 `--lh-*`** · 边框线 · 阴影（由权威表同步） | 同上 |
...
| 章节 | 同步目标文件 |
|------|-------------|
| **一、系统变量**（元素尺寸、布局尺寸、图标、字体字阶+行高、边框线、阴影） | `SystemVariable.css` |
| **二、主题变量**（标准色、功能色、辅助色、文本色） | `ThemeVariable.css` |

表格列约定：

- 系统变量（尺寸 / 布局 / 图标 / 边框 / 阴影）：`场景名称 | 尺寸或值 | 变量`
- 系统变量（§1.4 字体）：`场景名称 | 尺寸 | 字阶变量 | 行高变量`（`--fs-*` 与 `--lh-*` 强制成对）
- 主题变量：`场景名称 | 颜色 | 变量`
...
| 布局 | `--layout-header` · `--layout-content` | 页头/页脚/侧栏/版心，见 §1.2 |
| 字阶 | `--fs-14` · `--fs-20` | 与使用场景注释一致；须成对行高 |
| 行高 | `--lh-22` · `--lh-28` | 与同行字阶强制配对，见 §1.4 |
| 图标 | `--wh-16` · `--wh-24` | 宽高同值 |
