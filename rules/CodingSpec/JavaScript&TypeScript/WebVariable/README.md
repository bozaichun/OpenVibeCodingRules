...
| 文件 | 职责 | 谁改 |
|------|------|------|
| **`SubjectAuthority.md`** | **权威源**：主题色、文本色、间距、字号、图标、边框、阴影的表格定义 | 设计师 / 开发者先改这里 |
| **`ThemeVariable.css`** | 主题色 · 功能色 · 辅助色 · 文本色（由权威表同步） | 由 Agent / 开发者按表生成，勿手改漂移 |
| **`SystemVariable.css`** | 间距 · 图标 · 字号 · 边框线 · 阴影（由权威表同步） | 同上 |
...
| 章节 | 同步目标文件 |
|------|-------------|
| **一、系统变量**（元素尺寸、图标、字体、边框线、阴影） | `SystemVariable.css` |
