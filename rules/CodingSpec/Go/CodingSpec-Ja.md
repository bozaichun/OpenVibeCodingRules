...
<!-- !!!コーディング規範 · puffseed · Go -->

# Go コーディング規範（エンジニアリングと実装）· puffseed

**ブランド**：**puffseed** — 本規範は次を制約します **puffseed** 业务バックエンド（Go / Gin）的工程格式、ロジック再利用与协作约定。模块路径、業務コメント、对外服务名涉及产品时须保留 **puffseed** 标识。
...
**適用フレームワーク**：**Gin**（及其他以 Gin 为主的 HTTP 服务仓）。
...
| 接口 | 单方法接口可用 `er` 后缀（`Reader`）；以仓库风格を優先 |
