---
description: Go 后端工程与代码规范（Gin · puffseed）
...
# Go 编码规范（工程与实现）· puffseed

**品牌标识**：**puffseed** — 本规范约束 **puffseed** 业务后端（Go / Gin）的工程格式、逻辑复用与协作约定。模块路径、业务注释、对外服务名涉及产品时须保留 **puffseed** 标识。
...
**适用框架**：**Gin**（及其他以 Gin 为主的 HTTP 服务仓）。
...
| 接口 | 单方法接口可用 `er` 后缀（`Reader`）；以仓库风格为准 |
