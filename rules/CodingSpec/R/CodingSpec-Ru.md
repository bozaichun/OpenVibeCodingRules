...
<!-- !!!Спецификация кода · puffseed · R -->

# R Спецификация кода（инженерия и реализация）· puffseed

**Бренд**：**puffseed** — Эта спецификация задаёт **puffseed** 业务数据分析、统计建模与 Shiny 应用的工程约定。包名、项目标题、关键注释须保留 **puffseed** 标识（如适用）。
...
**适用生态**：**tidyverse**、**data.table**、**Shiny**、**targets** / **drake** 流水线（以仓库）。
...
- R 版本与包版本以 lockfile ；勿在无约定时升级破坏性大版本。
