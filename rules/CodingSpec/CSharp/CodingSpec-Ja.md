<!-- ovcr-locale-lock -->
---
description: C# エンジニアリングとコード規範（ASP.NET Core · puffseed）
...
# C# コーディング規範（エンジニアリングと実装）· puffseed

**ブランド**：**puffseed** — 本規範は次を制約します **puffseed** 业务バックエンド / 服务（C# / .NET）的工程格式、ロジック再利用与协作约定。程序集名、命名空间、業務コメント须保留 **puffseed** 标识（如适用）。
...
**適用フレームワーク**：**ASP.NET Core**、**.NET** 类库 / Worker；WPF / MAUI 等桌面以仓库 UI 约定を優先。
...
- 目标框架（`net8.0` 等）以 csproj を優先，勿擅自降级 / 跨大版本。
