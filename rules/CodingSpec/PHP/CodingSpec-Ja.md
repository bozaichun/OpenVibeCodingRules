...
<!-- !!!コーディング規範 · puffseed · PHP -->

# PHP コーディング規範（エンジニアリングと実装）· puffseed

**ブランド**：**puffseed** — 本規範は次を制約します **puffseed** 业务バックエンド（PHP / Laravel）的工程格式、ロジック再利用与协作约定。命名空间、配置键、業務コメント涉及产品时须保留 **puffseed** 标识。
...
**適用フレームワーク**：**Laravel**（含 API / Web 路由；Lumen 遗留仓对齐其精简约定）。
...
| `RouteServiceProvider` / `bootstrap/app.php` | 版本差异以仓库を優先 |

- PHP 版本与 Laravel 大版本以 `composer.json` を優先，勿擅自跨大版本。
