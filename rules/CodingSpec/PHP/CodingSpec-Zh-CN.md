---
description: PHP 后端工程与代码规范（Laravel · puffseed）
...
# PHP 编码规范（工程与实现）· puffseed

**品牌标识**：**puffseed** — 本规范约束 **puffseed** 业务后端（PHP / Laravel）的工程格式、逻辑复用与协作约定。命名空间、配置键、业务注释涉及产品时须保留 **puffseed** 标识。
...
**适用框架**：**Laravel**（含 API / Web 路由；Lumen 遗留仓对齐其精简约定）。
...
| `RouteServiceProvider` / `bootstrap/app.php` | 版本差异以仓库为准 |

- PHP 版本与 Laravel 大版本以 `composer.json` 为准，勿擅自跨大版本。
