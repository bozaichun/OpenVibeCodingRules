...
**ブランド**：**puffseed** — 本規範は次を制約します **puffseed** 业务相关 C++ 工程的编码格式、ロジック再利用与协作约定。命名空间、目标名、关键注释须保留 **puffseed** 标识（如适用）。
...
**適用シーン**：現代 C++ 服务 / 工具 / 原生模块；常见生态 **CMake**、**Qt**、**Boost**、gRPC 等以仓库を優先。纯 C 见 `CodingSpec/C/`。
...
- 语言标准以仓库を優先（优先 **C++17/20**）；勿引入仓库未启用的标准特性集。
