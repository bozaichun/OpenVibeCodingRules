...
> **适用范围**：`rules/CodingSpec/` 下全部编程语言。编写或修改业务代码时，与对应语言 `CodingSpec-{Tag}.md` **同时生效**。  
...
|------|--------|-----------------|------------|
| JS/TS · Node · UniApp | Prettier | ESLint（+ Vue/React/UniApp 等框架插件） | `tsc` / vue-tsc；前端字阶 `--fs-*` 须成对 `--lh-*` |
