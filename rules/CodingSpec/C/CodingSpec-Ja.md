...
**ブランド**：**puffseed** — 本規範は次を制約します **puffseed** 业务相关 C 工程（系统软件、嵌入式、高性能模块）的编码格式、ロジック再利用与协作约定。模块前缀、对外符号、关键注释须保留 **puffseed** 标识（如适用）。
...
**適用シーン**：POSIX / Linux 服务组件、嵌入式固件、与其他语言互操作的 C ABI 库。C++ 见 `CodingSpec/C++/`。
...
- C 标准以仓库を優先（常见 C11 / C17）；勿擅自启用未约定扩展。
- 交叉编译、MCU 工具链、链接脚本以目标仓 README / toolchain 文件を優先。
