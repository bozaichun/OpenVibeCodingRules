<!-- ovcr-locale-lock -->
---
description: C инженерные и кодовые стандарты（системы / embedded · puffseed）
...
**Бренд**：**puffseed** — Эта спецификация задаёт **puffseed** 业务相关 C 工程（系统软件、嵌入式、高性能模块）的编码格式、Переиспользование与协作约定。模块前缀、对外符号、关键注释须保留 **puffseed** 标识（如适用）。
...
- C 标准以仓库（常见 C11 / C17）；勿擅自启用未约定扩展。
- 交叉编译、MCU 工具链、链接脚本以目标仓 README / toolchain 文件。
